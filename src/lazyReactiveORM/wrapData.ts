...
export function makeTrap(target = {}, excludeProperties: string[] = []): Array<AbstractData | Subject<ModelEvent>> {
   ...
   const data = new Proxy<AbstractData>(target, {
      ...
      get(target: AbstractData, property: string | number | symbol): any {
         ...
         if (excludeProperties.some((key) => key === property)) {
            return null
         }
...
export function makeTrapBySchemas(
   ...
   const [resultTrap, resultStream] = makeTrap(target, excludeProperties)
   ...
   Object.keys(schema).forEach((key) => {
      ...
      const [trap, stream] = makeTrap({}, excludeProperties)
      stream
         .pipe(
